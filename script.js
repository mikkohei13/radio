class AudioVisualizer {
    ...
    // Convert position (0-1) to rainbow color (red -> orange -> yellow -> green -> blue -> violet)
    getRainbowColor(position) {
        // Clamp position between 0 and 1
        position = Math.max(0, Math.min(1, position));
        
        // Map position to hue (0 = red, 1 = violet)
        // Red is at 0°, violet is at ~270° (or -90°), so we go from 0 to 270
        const hue = position * 270;
    ...
    draw() {
        ...
        for (let i = 0; i < dataLength; i++) {
            ...
            // Calculate rainbow color based on horizontal position
            const position = i / dataLength; // 0 to 1 across the canvas
            const baseColor = this.getRainbowColor(position);
            
            // Create vertical gradient for each bar (from lighter at top to base color at bottom)
